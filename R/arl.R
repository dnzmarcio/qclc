#' Operating Characteristics for MA
#'
#' This function generates the Operating Characteristics of the Moving Average (MA) quality control charts.
#'
...
#' @param L A numeric value or vector. \code{L} is the control limit factor; represents 
...
#'
#' @return A data frame of the Operating Characteristics for the EWMA charts: 
#' Control Limit Factor \code{L}, Average Run Length \code{ARL}, 
#' and either the False Positive rate \code{FP} (when the process is in control) 
#' or the True Positive rate \code{TP} (when the process is out of control).
#' 
#' @export
#'
#' @details
#' \code{ARL} is the expected Run Length \code{rl} generated by \code{arl_ewma}. 
#' Average Run Length is denoted as \code{ARL0} when the process is under control, 
#' and \code{ARL1} when the process is out of control. \code{ARL0} and \code{ARL1} refer
#' to the expected number of observations to detect an out-of-control condition. 
#' When process is under control, a high ARL0 is preferred as it demonstrates 
#' that the chart does not react excessively to regular variations within process 
#' limits, thus minimizing false alarms. Conversely, a low ARL1 is 
#' desirable because it signifies that the chart is adequately sensitive, 
...
#' arl_ma(x, mu = aux$mu, aux$sigma2, omega, 
#'        L = seq(1, 3, by = 0.25), 
#'        max.rl = 110, rl = 100, 
...
#' Calibration Parameters for EWMA 
#'
#' This function generates the Operating Characteristics of the Exponential Weighted 
#' Moving Average (EWMA) quality control charts.
#' 
#' @param x A numeric vector of individual observations obtained from the process.
#' In some cases, \code{x} can also indicate sample averages according to a specified sampling plan.
#' @param mu double; process mean. Also known as the target mean or the average of the historical data.
...
#'
#' @return A data frame of the Operating Characteristics for the EWMA charts: 
#' Control Limit Factor \code{L}, Average Run Length \code{ARL0}, 
#' False Positive rate \code{FP} (when the process is in control), and
#' Average Run Length \code{ARL1}, True Positive rate \code{TP} 
...
#' 
#' ## process under control
#' arl_ewma(x, mu = aux$mu, aux$sigma2, lambda, 
#'          L = seq(1, 3, by = 0.25), 
#'          max.rl = 110, rl = 100, 
#'          scenario = 0, nboot = 100, 
#'          ncores = 1, seed = 1234)
